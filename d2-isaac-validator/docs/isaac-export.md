...
> **Bounty kit note:** this is the internal design + status doc, reproduced as-is for Bounty
> D2 (see the kit `README.md` for the actual task). The cross-refs below (`gotchas.md`,
> `code-structure.md`, `conventions.md`) are sibling docs in the private product repo and are
> **not included in this kit** — they aren't needed for the validator task; treat §2/§5/§6
> (what it writes, the code map, the invariant checklist) as the spec to validate against. §3's
> broker-route material is background context, not something this bounty runs.
