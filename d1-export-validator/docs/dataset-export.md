...
> **Bounty kit note:** this is the internal design + as-built spec, reproduced as-is for
> Bounty D1 (see the kit `README.md` for the actual task). The cross-refs below
> (`conventions.md`, `gotchas.md`, `scene-report.md`, `code-structure.md`) are sibling docs in
> the private product repo and are **not included in this kit** — they aren't needed for the
> validator task; treat the schema tables and §7/§9 invariants in this file as the complete
> spec to validate against. §11.4's GPU/Modal harness is background context, not something
> this bounty runs.
...
shared weights-cache and I/O Modal Volumes (internal Volume names omitted here).
...
- Writes seed/frames/exports under the shared I/O Modal Volume — point at a
