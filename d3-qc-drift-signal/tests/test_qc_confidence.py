...
NOTE (bounty kit adaptation): this file is copied verbatim from the source
repo's tests/test_qc_confidence.py except for import paths (server.qc.* ->
qc.*) and one removed test — see TestDegenerateTrajectoryFiles below."""
...
class TestSynthetic:
    ...
    def test_unmeasurable_sign_repairs_skip_the_signal(self, tmp_path: Path) -> None:
        ...
        signal died once already (see server/oreos/pose_qc.py).
        """
        ts, pos = circle_trajectory()
...
class TestDegenerateTrajectoryFiles:
    ...
    server/oreos/modal_recon.py writes results/est_tum.txt unconditionally, so a
    ...
    def test_load_tum_shape_is_nx3_even_when_empty(self, tmp_path: Path) -> None:
        from qc.confidence import load_tum
    ...
    # NOTE (bounty kit adaptation): the source suite has one more test here,
    # test_pipeline_qc_stage_refuses_instead_of_aborting, which drives this
    # refusal contract end-to-end through the full Oreos ingest/export
    # pipeline (`server/oreos/pipeline.py: OreosPipeline`). That pipeline is a
    # much larger subsystem (ingest, export orchestration, its own DB/session
    # state) that is out of scope for this kit — only qc/confidence.py was
    # pulled out. The refusal contract itself (degenerate trajectory file ->
    # OUT_OF_CLASS, never a crash) is still fully exercised by the other
    # tests in this class, which call compute_confidence directly.
...
class TestQcCli:
    """qc/cli.py — the shell-facing gate (exit 0/1/2)."""
    ...
    def _run(self, monkeypatch, argv: list[str]) -> tuple[int, dict]:
        ...
        printed: list[str] = []
        monkeypatch.setattr(sys, "argv", ["qc.cli", *argv])
